import { Sprout, PlayCircle, Users, Leaf } from 'lucide-react';
...
export function SimpleHomeScreen({ navigateTo, goBack, canGoBack, currentUser, notifications, setCurrentUser }: SimpleHomeScreenProps) {
  return (
    <div className="min-h-screen bg-[#faf9f7] relative overflow-hidden flex flex-col" style={{ backgroundImage: 'radial-gradient(circle, #e5e7eb 1px, transparent 1px)', backgroundSize: '20px 20px' }}>
      ...
      <SimpleNav currentScreen="home" navigateTo={navigateTo} currentUser={currentUser} notifications={notifications} setCurrentUser={setCurrentUser} />
      
      <div className="max-w-2xl mx-auto px-6 py-24 text-center relative z-10 flex-1">
        ...
        {/* Title */}
        <h1 className="text-6xl md:text-8xl font-extrabold text-[#3d2817] mb-6 leading-tight uppercase tracking-tight transform hover:scale-105 transition-transform duration-300">
          Grow Knowledge<br />Together
        </h1>
        <p className="text-xl text-[#3d2817] mb-12 max-w-md mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
          <Button
            ...
            className="bg-[rgb(213,192,93)] hover:bg-[rgb(193,172,73)] text-white h-16 px-12 rounded-full border-4 border-[#3d2817] text-xl font-extrabold uppercase shadow-2xl hover:shadow-3xl hover:scale-105 transition-all duration-300 transform hover:-rotate-2 text-[24px]"
          >
